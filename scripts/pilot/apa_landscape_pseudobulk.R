...
if (length(args) < 2) {
  stop("Usage: apa_landscape_pseudobulk.R <sce_rds> <output_dir>")
}
...
if (!"sample_id.sq" %in% names(colData(sce))) {
  stop("colData(sce) does not contain sample_id.sq")
}
sample_ids <- as.character(colData(sce)$sample_id.sq)
cell_ids <- if ("cell_id" %in% names(colData(sce))) {
  as.character(colData(sce)$cell_id)
} else {
  colnames(counts_mat)
}
...
# Pseudobulk counts: features x donors
G <- sparseMatrix(
  i = seq_along(sample_ids),
  j = as.integer(sample_factor),
  x = 1,
  dims = c(length(sample_ids), length(sample_levels))
)

pb_counts <- counts_mat %*% G
colnames(pb_counts) <- sample_levels
rownames(pb_counts) <- rownames(counts_mat)

saveRDS(pb_counts, file.path(outdir, "pseudobulk_counts_matrix.rds"))

# Donor / sample summary from the original cell-level matrix
...
donor_total_umis <- as.numeric(Matrix::colSums(pb_counts))
donor_detected_utrs <- as.numeric(Matrix::colSums(pb_counts > 0))

donor_summary <- cell_counts %>%
  group_by(sample_id) %>%
  summarise(
    n_cells = n(),
    total_umis_from_cells = sum(cell_total_umis, na.rm = TRUE),
  ...
  mutate(
    donor_total_umis = donor_total_umis[match(sample_id, sample_levels)],
    donor_detected_utrs = donor_detected_utrs[match(sample_id, sample_levels)]
  ) %>%
  arrange(desc(donor_total_umis))

# UTR-level summary on donor-pseudobulk counts
utr_tbl <- rd %>%
  mutate(
    total_counts = as.numeric(Matrix::rowSums(pb_counts)),
    detected = total_counts > 0,
    valid_feature = !is_blacklisted & !is_ipa,
    donors_detected = as.numeric(Matrix::rowSums(pb_counts > 0))
  ) %>%
  group_by(gene_name) %>%
  mutate(
...
# Write outputs
write.csv(donor_summary, file.path(outdir, "donor_summary.csv"), row.names = FALSE)
